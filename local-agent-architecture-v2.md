...
graph TD
    ...
    K -->|"Neither"| H["Model invents it"]

    W --> OK["Real answer"]
    R --> OK
    H --> BAD["Fabricated image names<br/>wrong ports · fake versions<br/>presented confidently"]
```

These fix *different* hallucinations, and web search will never cover the second column —
your own infra details aren't on the internet. Tavily was the original path for column one
and is now superseded by self-hosted SearXNG (no API key, no rate limit, nothing leaves
the LAN).

The observed failure mode is that a model invents a single Docker image and a plausible
port for a project that's actually a multi-service compose stack, then quotes two or three
different "default" ports across the same session. Grounding is what stops the guess.
...
1. **Scope the SSH-enabled agent deliberately.** An agent that can SSH *and* occasionally
   over-claims success should only reach hosts you've decided it can reach.
